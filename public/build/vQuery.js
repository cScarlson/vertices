...
/******/ 	var hotCurrentHash = "7c79f7d3460a550bb99f"; // eslint-disable-line no-unused-vars
...
;(function save() {
    function defaultBootstrap(root, medium) {
        ...
        var childMedium = medium.$spawn(new function ChildScope() {
            this.element = root;
        }());
...
;(function iif(undefined) {
    ...
    var utils = new function Utils() {
        ...
        function extend(target) {
            ...
            var object = objects.shift(),
                args = [target].concat(objects);

            for (var key in object) {
                var value = object[key];
                target[key] = target[key] || {};
                if ({ 'Object': true }[value.constructor.name]) extend(target[key], value);else target[key] = value;
            }
        ...
        function interpolate(str) {
            return function interpolate(o) {
                return str.replace(/{([^{}]*)}/g, function (a, b) {
                    var r = o[b],
                        val = typeof r === 'string' || typeof r === 'number' ? r : a;
    ...
    var Core = function Core($) {
        ...
        var configuration = { // ... defaults
            ...
            decorators: {
                services: function DefaultServicesSandbox(utils) {
                    return utils;
                },
                components: function DefaultComponentSandbox(element) {
                    return element;
                }
            }
        };
    ...
    var Facade = function Facade(core) {
        ...
        function register(id, Component) {
            var method = { '$component': 'component', '$service': 'service' }[id],
                method = method || { 'function': 'service', 'string': 'component' }[typeof id === 'undefined' ? 'undefined' : _typeof(id)];
...
;(function iif(V) {
    ...
    var Mediator = function Mediator(EventHub) {
        ...
        function publish(channel, data) {
            ...
            if (channel in this.publishers) this.publishers.call(this, channel, data);else hub.publish.apply(hub, arguments);
        ...
        function subscribe(channel, handler) {
            ...
            if (channel in this.subscribers) this.subscribers.call(this, channel, data);else hub.publish.apply(hub, arguments);
    ...
    var ComponentSandbox = function ComponentSandbox(element) {
        ...
        var Scaffold = function Scaffold(element) {
            // Interface
            var thus = this;
            ...
            function bootstrap(options) {
                // V.bootstrap should only be available to Components
                V.bootstrap(options);
                return this;
            }
        ...
        var f = Scaffold.call(function f(selector) {
            // Default action
            var $element = f.element.find(selector),
                element = $element[0];
        ...
        bootstrap: function handleBootstrap(target, core) {
            ...
            function mountComponent(component) {
                ...
                var instance = component.instance,
                    handleMount = instance.handleMount;
    ...
    V('vInterpolate', function VInterpolate($) {
        // vInterpolate is used for {variable} bindings
        var thus = this;

        function init(data) {
            var template = $.target.innerHTML,
                $variables = data.dataset.vinterpolate || '{}',
    ...
    V('vRepeat', function VRepeat($) {
        ...
        function repeat(template, item, i) {
            var $element = $.create(template),
                element = $element[0],
                $ids = element.dataset.v,
                json = JSON.stringify(item);
            var _ids = $ids.split(' '),
                index = _ids.indexOf('vRepeat'),
                _r = _ids.splice(index, 1),
                ids = _ids.join(' ');
        ...
        function init(data) {
            var $template = $.element,
                target = $.target,
                template = target.outerHTML,
                items = JSON.parse(target.dataset.vrepeat);
            var $comment = $.create('<!-- vRepeat -->'),
                elements = items.map(repeat.bind(this, template));
    ...
    V('vAttributes', function VAttributes($) {
        // vAttributes is used for Attribute-Directives
        var thus = this;
